...
from ._base_animate import save_animation
...
class BaseWindows(BaseModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        batch_index = 0
        pred = output[batch_index].detach().cpu().numpy()
        gt = np.concatenate(
                (insample_y[batch_index].cpu().numpy(), 
                 outsample_y[batch_index].cpu().numpy()))
        
        if os.environ["START"] != "1" and os.environ["STEP"] != "1" and os.environ["VISUALIZE"]=="1":
            fractional_lines = []
        ...
        for fraction in torch.arange(float(os.environ["START"]), 1, float(os.environ["STEP"])):
            ...
            if os.environ["START"] != "1" and os.environ["STEP"] != "1" and os.environ["VISUALIZE"]=="1":
                # fractional GT and pred
                fractional_lines.append([insample_y_new[batch_index].detach().cpu().numpy(), output_pred[batch_index].detach().cpu().numpy(), fraction])
        ...
        if os.environ["START"] != "1" and os.environ["STEP"] != "1" and os.environ["VISUALIZE"]=="1":
            if self.current_epoch % 10 == 0:
                try:
                    save_animation(gt, pred, fractional_lines, fname="epoch_%d_start_%s_step_%s" % (self.current_epoch, os.environ["START"], os.environ["STEP"]))
                except ValueError:
                    print ("Couldn't save epoch %d because it wasn't the main thread!" % self.current_epoch)
                    pass
