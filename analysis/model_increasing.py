...
def forecast(input_size, h):
    ...
    for m in model_names:
        fname = "results_lookahead_increasing_%s/%d_%s_%s_%s.png" % (m, h, os.environ["START"], os.environ["STEP"], os.environ["LAMBDA"])
    ...
    # trend data already reshaped
    #if h > df["y"].shape[0]:
    #    df = create_trend_data(data_size=h, spacing=100)
    #    # normalize
    #    MEAN, STD = df["y"].mean(), df["y"].std()
    #    df["y"] = (df["y"] - MEAN) / STD
...
#        return
    ...
    for model_name in model_names:
        ...
        if not os.path.isdir("results_lookahead_increasing_%s" % model_name):
            os.mkdir("results_lookahead_increasing_%s" % model_name)
        plt.savefig("results_lookahead_increasing_%s/%d_%s_%s_%s.png" % (model_name, h, os.environ["START"], os.environ["STEP"], os.environ["LAMBDA"]))
        plt.cla()
        plt.clf()
        with open('results_lookahead_increasing_%s/metrics.csv' % model_name, 'a') as csvfile:
